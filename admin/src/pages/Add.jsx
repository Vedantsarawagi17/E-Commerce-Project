...
export const Add = ({token}) => {
  ...
  const onSubmitHandler = async(e) => {
    ...
    try {
      ...
      // Append lets you bundle them all together
      ...
      // Turn array into a flat string using " JSON.stringify() "
      // Used to send data to Database . 
      formData.append("sizes", JSON.stringify(sizes))

      // JSON.parse() is used to make string back into a Functional JavaScript array.
      // Used to read data from Database .

      // This specific block of code is a safety check.
      // If image exists, Then append it to the form data.
      // formData.append("key",value)
      formData.append("image1",image1) && image1
      formData.append("image2",image2) && image2
      formData.append("image3",image3) && image3
      formData.append("image4",image4) && image4
      ...
      <div>
        ...
        <div className='flex gap-2 '>
           ...
           <label htmlFor="image2">
            <img className='w-20' src={!image2 ? assets.upload_area : URL.createObjectURL(image2)} alt="image-2" />
            <input onChange={(e)=>setImage2(e.target.files[0])} type="file" id='image2' hidden />
           </label>

           <label htmlFor="image3">
            <img className='w-20' src={!image3 ? assets.upload_area : URL.createObjectURL(image3)} alt="image-3" />
            <input onChange={(e)=>setImage3(e.target.files[0])} type="file" id='image3' hidden />
           </label>

           <label htmlFor="image4">
            <img className='w-20' src={!image4 ? assets.upload_area : URL.createObjectURL(image4)} alt="image-4" />
      ...
      <div>
         ...
         <div className='flex gap-3  '>
          ...
          {/* <div onClick={()=>setSizes(prev => prev.includes("XS") ? prev.filter(item => item !== "XS") : [...prev,"XS"])}>
            <p className={`${sizes.includes("XS") ? "bg-green-300" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>XS</p>
          </div> */}
